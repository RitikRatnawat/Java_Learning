package com.example.jdbc_gui.model;
...
public class DataSource
{
    ...
    public List<Album> queryAlbumsforArtistID(int id)
    {
        try
        {
            ...
            ResultSet data = queryAlbumByArtistID.executeQuery();

            List<Album> albums = new ArrayList<>();
            while(data.next())
            {
                Album album = new Album();
                album.setId(data.getInt(1));
                album.setName(data.getString(2));
    ...
    public boolean updateArtist(String newName, int id)
    {
        try
        {
            updateArtistName.setString(1, newName);
            updateArtistName.setInt(2, id);
            int affected = updateArtistName.executeUpdate();
